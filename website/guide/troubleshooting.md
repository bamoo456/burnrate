...
## Building from source fails: `ld: library not found for -liconv`
...
a machine **every** Rust binary build fails the same way — not just Burnrate.

The repo's own `.cargo/config.toml` pins the linker, which covers a plain
`cargo build` in a clone. If you build outside the repo (`cargo install --git`
with an unusual `$CARGO_HOME`, for instance), add the same pin to
`~/.cargo/config.toml`:
...
```sh
CARGO_TARGET_AARCH64_APPLE_DARWIN_LINKER=/usr/bin/cc cargo build --release
```

(use `CARGO_TARGET_X86_64_APPLE_DARWIN_LINKER` on Intel Macs). The native
bundles and `nix run` are unaffected either way.

## Building from source fails: `unrecognized command-line option` (GCC as `cc`)
...
```sh
CC=/usr/bin/cc CXX=/usr/bin/c++ cargo build --release
```
...
Or use **Sign in again** on the account in Preferences. That re-authenticates
the account in its own isolated CLI dir; it never touches your system-default
`~/.claude` session.
...
Burnrate never refreshes your system-default `~/.claude` login; your own
`claude` sessions keep that one fresh.

## Headless diagnostics

The binary ships a hidden debug CLI that exercises the real config and fetch
code paths without the GUI:

```sh
burnrate debug env       # credential-override env vars + resolved provider CLIs
burnrate debug load      # full startup load + orphan GC
...
fetch from your machine.

`burnrate debug detect` and `burnrate debug insights` still exist but always
return empty in this fork: account auto-detection and local session scanning
are both removed. To sandbox the account database, set
`BURNRATE_CONFIG_DIR=$(mktemp -d)` — provider credentials still come from
